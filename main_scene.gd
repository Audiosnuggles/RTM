...
# Sie bündelt die Logik zur Aktualisierung der UI-Elemente.
func _update_fragment_display():
    ...
    if is_instance_valid(Combat) and is_instance_valid(fragment_display):
        # Aktualisiert die "Coins"-Anzeige
    ...
    if is_instance_valid(power_display):
        # Aktualisiert die Leistungsanzeige (Passiv und Klick)
        power_display.text = (
...
func _process(_delta):
    ...
    if is_instance_valid(fragment_display) and fragment_display.visible:
        _update_fragment_display() # <-- Dieser Aufruf ersetzt die alte Logik
...
func _on_upgrade_button_pressed():
    if Combat.upgrade_healing_power():
        _update_fragment_display() # <-- Dieser Aufruf funktioniert jetzt
